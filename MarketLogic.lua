...
function MarketScreenDisplayCategory( screen, categoryIndex, args )
	...
	for itemIndex, item in ipairs( items ) do
		...
		if not item.SoldOut and ResourceData[item.BuyName] ~= nil then
			...
			item.Showing = true
			if not category.AllowPurchaseWithoutResources and not HasResources( item.Cost ) then
...
function HandleMarketPurchase( screen, button )
	...
	local spendArgs = { ApplyMultiplier = false, OffsetX = 600, Silent = true }
	if category.AllowPurchaseWithoutResources then
		spendArgs.AllowNegative = true
	end
	if category.GrantCostRewards then
		local addArgs = { ApplyMultiplier = false, OffsetX = 600, Silent = true }
		...
		AddResource( item.BuyName, item.BuyAmount, "ResourceShop", addArgs )
		for resourceName, resourceAmount in pairs( item.Cost ) do
			if resourceAmount ~= 0 then
				local rewardAmount = resourceAmount
				if rewardAmount < 0 then
					rewardAmount = -rewardAmount
				end
				AddResource( resourceName, rewardAmount, "ResourceShop", addArgs )
			end
		end
	...
	SpendResources( item.Cost, "ResourceShop", spendArgs )
